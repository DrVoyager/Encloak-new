...
public class Transformer {
	...
	Map<Value, Integer> SenstiveStaticFieldArray = new HashMap<>();
	Map<Value, Integer> SenstiveStaticFieldIndexArray = new HashMap<>();
	Map<Value, Value> SenstiveFieldStaticCuuidArray = new HashMap<>();
	...
	public Transformer(Body aBody, String phase,
		...
		/**
		 * new test , we don't care about clinit(why?)
		 */
		...
		while (scanPre.hasNext()) {
			...
			// load into each condVal list(defbox)
			...
			// load into each condVal list(usebox)
		...
		// ----------------------------------------------------------------------------------------------------------
		G.v().out.println("dealWithCuuid");
		boolean res = declaredMethodName.equals("<init>");
		G.v().out.println("declaredMethodName: " + declaredMethodName + "; res: " + res);
		// deal with Cuuid
		if (declaredMethodName.equals("<init>")
			...
			G.v().out.println("Cuuid Classname:" + declaredClassName
					+ " declaredFunction:" + declaredMethodName);
			if (!memberVariables.isEmpty() || !staticmemberVariables.isEmpty()) {
				...
				while (scanIt1.hasNext()) {
					...
					G.v().out.println("CuuidStmt" + CuuidStmt);
		...
		while (scanIt2.hasNext()) {
			...
			if ((currProStmt instanceof IdentityStmt)) {
				...
				// &&
				// INVOKEMAP.get(declaredClassName).get(declaredMethodName)[Integer.parseInt(((IdentityStmt)
				...
				if (CFMAP.containsKey(declaredClassName)
					...
					if (CFMAP.get(declaredClassName).get(declaredMethodName)
						...
						G.v().out.println("IdentityStmt: isSenstiveflag "
								+ isSenstiveflag + " declaredMethodName:"
								+ declaredMethodName);
						// if (declaredMethodName.equals("<init>")) {
...
//						}
	...
	private Unit replaceValueUpdateStmt(Body aBody, Local sgxObjLocal,
		...
		// int a=arr[0] 右边是一维数组引用类型（数组+下标）
			...
			boolean res1 = rightOp instanceof ArrayRef;
			G.v().out.println("res1: " + res1);
			boolean res = rightOp instanceof InstanceFieldRef;
			G.v().out.println("rightOp instanceof InstanceFieldRef: " + res);
			...
			if (memberVariables.containsKey(aBody.getMethod()
				...
				G.v().out.println("declaringclassName: " + aBody.getMethod()
						.getDeclaringClass().getName());
			...
			G.v().out.println("rightOp instanceof StaticFieldRef");
			SootField sField = ((StaticFieldRef) rightOp).getField();
			G.v().out.println("sField.name: " + sField.getName());
			G.v().out.println("=======G=======" + rightOp.toString());

			if (staticmemberVariables.containsKey(aBody.getMethod()
					.getDeclaringClass().getName())
					&& staticmemberVariables.get(
							aBody.getMethod().getDeclaringClass().getName())
							.contains(sField.getName())) { // this sootfield is sensitive
				...
				G.v().out.println("static sootfield is not sensitve");
